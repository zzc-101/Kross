...
describe('replayTraceEvents', () => {
  it('strictly derives a frame timeline without executing side effects', () => {
    ...
    expect(replay.frames[2]).toMatchObject({ activeToolCalls: 1 });
    expect(replay.frames[3]).toMatchObject({ activeToolCalls: 0 });
  ...
  it('rejects mixed runs and duplicate event ids', () => {
    const mixed = [
      event(0, 'run.started', { input: 'x' }),
      { ...event(1, 'context.built', {}), runId: 'run-2' }
    ];
    ...
    expect(() =>
      replayTraceEvents('run-1', [
        first,
        { ...event(1, 'context.built', {}), id: first.id }
      ])
    ).toThrow(
