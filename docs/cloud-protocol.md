...
- `GET /api/v2/agent/workspace/file/content?path=`（登录 cookie 鉴权后 302 到预签名 GET；组织 ID 可用请求头或 `organizationId` query，供 `<img>` / 下载；默认 `inline=true`；同一路径约 2 分钟内复用已签名 URL）
...
- `agent.command` / `agent.command_result`：受限工作区 `list` / `read` / `write`（内部 UTF-8）、`pull` / `push`（预签名 HTTP 同步对象存储，单文件 10MB，仅允许内部 S3 host）、`delete` / `mkdir`，以及记忆提取。路径继续走 Worker realpath jail。
