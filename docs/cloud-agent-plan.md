...
- **P2 增强**:Web Push 审批通知(移动端锁屏可批)、Git 分支/push/PR 流、容器空闲回收、资源限额与原生 App 接入(直接复用 protocol 包)。其中容器默认限额已提前落地为 1 CPU、2 GiB 内存、256 PID 和 10 GiB 应用层磁盘软限额。
...
| P2 | Web Push 审批通知、Git push/PR 流、容器空闲回收、资源限额（默认值与磁盘软限额已完成） |
