...
5. 独立 Sandbox 当前不在产品计划内；本地依赖审批，Cloud 依赖 Worker 容器边界。
...
- 修订融合稿中的事实、优先级和依赖关系；
- 记录本地审批与 Cloud Worker 容器的现有执行边界；
- 新增本实施路线；
- 保留各模型原始草案作为决策输入。
...
> 第一版已完成：真实模式要求显式 Case、Provider、模型与总预算；支持顺序重复
> 运行、脱敏结构化报告和 Provider Matrix。普通 CI 仍只运行 Fixture。统计置信
> 区间、`pass@k`、held-out Case 管理和并发调度留待后续。
...
- 独立命令级 Sandbox 或 Runtime Sandbox Adapter；
- Kross-as-MCP-server；
- 公开模型排行榜；
- 独立 GUI 客户端；
- 通用 Workflow 编排器；
