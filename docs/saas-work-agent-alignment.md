...
- [x] 11. 统一运行 Worker 和前端的类型检查、测试与构建；后端由用户运行验证。

最终验证记录：

- Frontend：typecheck 通过；5 个测试文件、16 个测试通过；Web 与 Admin Web 构建通过。
- Worker：typecheck 通过；71 个测试文件、314 个测试通过；运行时构建通过。
- 仓库：版本一致性与 25 个 Markdown 文件的链接检查通过。
- 当前执行环境为 Node.js 20.18.3，低于支持基线 22.19.0，命令虽全部通过，发布环境仍应使用支持版本复验。
- Backend：按项目约定未编译、未运行测试，由用户在 Java 21 运行环境验证。
