...
状态：P0 主闭环与 P1 已完成；P0-1d 暂缓，P2 为后续 backlog。
...
- P1-6 Run Checkpoint 与安全恢复：已完成（2026-07-18）。
- P1-7 工具调度优化：已完成（2026-07-18）。
...
- Session Work State 增加版本化 `runCheckpoint`，记录 run、mode、阶段、迭代、验证追问次数、已完成调用 id、待审批调用和后续队列；旧版无 checkpoint 会话保持兼容。
- 等待审批时同步持久化 open turn 与 checkpoint；恢复前核对 assistant tool call、已有 tool result 和当前工具清单，证据不一致则 fail-closed，不开放恢复入口。
- 只有尚未执行的待审批调用可以跨进程恢复。已完成调用只作为上下文和 checkpoint 证据存在，恢复路径不会重新调用；非幂等写入和执行工具禁止盲目重放。
- 退出时不再自动取消尚未执行的审批 checkpoint。恢复后审批面板重新出现：批准会从安全边界继续，拒绝或 Esc 可结束相应调用/运行。
- 普通崩溃中断但不处于审批边界的 run 仍按 interrupted turn 收口，不尝试猜测性重放。
...
- 主 Agent 与 complete-based 子代理共享风险感知调度规则：连续、独立、已通过策略预检的 read 调用最多 4 个并发，并按原 tool-call 顺序回填结果。
- write、execute、network、process、MCP 和需要审批的调用保持串行屏障；动态风险解析后不再满足 read 的调用不会进入并发组。
- `ProcessPoll` 跟踪输出与终态指纹；连续无变化时从 250ms 指数退避到最多 4s，新输出或进程结束会重置进展状态。
- 模型、工具和 MCP 失败统一输出 source、category、retryable 与 recovery；MCP `isError` 不再被记录为 completed。
