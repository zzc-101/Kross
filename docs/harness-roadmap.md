...
当前主要短板不是“缺少 Agent 能力”，而是 Harness 对完成质量的约束不足：主模型不再发起工具调用时，run 即可进入 `completed`；发生代码修改后，系统不会确认最后一次修改之后是否执行过有效验证；主 Agent 也缺少子代理已有的重复工具调用 stall guard。
...
- 对每轮 tool-call batch 生成稳定 signature。
- 连续重复同一 signature 时记录无进展次数。
- 达到阈值后不再执行重复调用，注入一次恢复提示，要求模型改变策略或总结阻塞原因。
- 恢复后仍重复则软着陆为“未完成/需要继续”，不得伪装为正常完成。
- 对用户输入、工具输出或调用参数发生实质变化的情况重置 stall 计数。
