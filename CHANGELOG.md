...
- 用户永久记忆：库表为真相，偏好写入 `USER.md`、事实写入 `MEMORY.md`；
  面板可记/改/忘记，对话「帮我记住」立刻落库，Worker 休眠前从用户发言巩固。
  抽取水位按本批最后一条用户消息前进；失败或未配模型不跳过后续发言。
...
- Worker 控制面协议升级到 v3：任务带可续租 lease；最终消息通过稳定
  `deliveryId`、持久去重记录及 ACK 实现断线重投，直播事件保持 best-effort。
...
- Worker 短暂重连不再把处理中任务直接重排或清空忙碌状态；过期任务由租约恢复，
  且离线审批会明确失败而不是静默丢失。
- 登录会话会在每次请求时刷新账号状态与平台角色，账号禁用或降权立即生效；登录
  失败路径始终执行密码哈希校验，新密码不再允许超过 BCrypt 的 72 字节边界。
- Agent 容器启动与休眠按 agent 串行化；已分配节点离线时禁止自动漂移，节点命令
  结果也必须来自原目标节点，避免重连和共享工作区双活。
