...
- 工作区存储可切换：默认本机 Docker volume；集群可将 `/work` 挂到 JuiceFS（数据仍在
  MinIO）。通过 `KROSS_WORKER_STORAGE` 与可选的 `docker-compose.juicefs.yml` 启用。
- 多机 Worker：独立 Go 服务 `kross-node`（`node/`）出站连控制面；
  `KROSS_WORKER_RUNTIME=cluster` 时控制面只调度，不本机起容器。JuiceFS 工作区可漂。
