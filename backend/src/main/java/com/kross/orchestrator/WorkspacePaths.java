...
public final class WorkspacePaths {
  ...
  public static Path requireMount(KrossProperties properties) {
    return Optional.ofNullable(properties.getJuicefsMount()).map(String::trim).filter(value -> !value.isBlank())
        .map(Path::of)
        .map(Path::toAbsolutePath)
        .map(Path::normalize)
        .orElseThrow(() -> new IllegalStateException("KROSS_JUICEFS_MOUNT is required when KROSS_WORKER_STORAGE=juicefs"));
  }

  public static Path agentDirectory(KrossProperties properties, String agentId) {
